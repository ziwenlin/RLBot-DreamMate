...
class TestTimedAction(TestCase):
    ...
    def test_run1(self):
        state = self.action.run(None, -1)
        self.assertFalse(self.value)
        self.assertFalse(state)

    def test_run2(self):
        state = self.action.run(None, -0.5)
        self.assertFalse(self.value)
        self.assertFalse(state)

    def test_run3(self):
        state = self.action.run(None, 0)
    ...
    def test_run6(self):
        state = self.action.run(None, 1.5)
        self.assertTrue(self.value)
        self.assertFalse(state)
    ...
    def test_zero(self):
        ...
        state = action.run(None, 0)
        self.assertTrue(self.value)
        self.assertFalse(state)

        state = action.run(None, 1)
        self.assertTrue(self.value)
